  ...
  function scanRevision({name, revision, bundleDir}) {
    ...
    let check = targets
      .map(name => {
        ...
        if ( ! enabled) {
          return {name, message:'SSLInfo configuration does not include Enabled flag'};
        }
        elts = xpath.select(`IgnoreValidationErrors`, sslInfo);
        let ignoreErrors = elts && elts[0] && elts[0].childNodes && elts[0].childNodes[0] &&
          elts[0].childNodes[0] == 'true';
        if (ignoreErrors) {
          return {name, message:'SSLInfo configuration includes IgnoreValidationErrors = false'};
        }
        return null;
      });
